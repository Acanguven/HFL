...
local ts = TargetSelector(TARGET_LESS_CAST, 1250, DAMAGE_PHYSICAL, false)
...
class 'esp'
	...
	function  esp:calculateMinions(node)
		...
		for n, m in pairs(MINIONS.allies.objects) do
			md.x = md.x + MINIONS.allies.objects[n].x
			md.z = md.z + MINIONS.allies.objects[n].z
		end
		for e, ee in pairs(MINIONS.enemies.objects) do
			em.x = em.x + MINIONS.enemies.objects[e].x
			em.z = em.z + MINIONS.enemies.objects[e].z
		end
		

		mdraw.x = (md.x  / #MINIONS.allies.objects) - 100
		mdraw.z = (md.z  / #MINIONS.allies.objects) - 100

		edraw.x = (em.x  / #MINIONS.enemies.objects)
		edraw.z = (em.z  / #MINIONS.enemies.objects)
...
class 'orb'
	function orb:__init()
		...
		AddProcessAttackCallback(function(unit, attackProc) 
			self:OnProcessSpell(unit,attackProc)
		end)
	...
	function orb:heroCanMove()
		return (GetTickCount() + GetLatency()/2 > self.lastAttack + self.previousWindUp + 20)
	end
...
class 'position'
	function position:__init()
		self.mdraw = {x=5940, z=6040}
		self.edraw = {x=0, z=0}
		self.underT = {
			AllyTower = false,
			EnemyTower = false
		...
		self.towers = {}
		self.lastMovement = clock()

		self:collectTowers()

		AddTickCallback(function ()
			self:calculateMinions()
		end)
		AddDrawCallback(function ()
			self:drawManager()
		end)
	...
	function position:calculateMinions()
		md = {x=0, z=0} 
		em = {x=0, z=0}
		if player.dead or GetGame().isOver then return end 
		for n, m in pairs(MINIONS.allies.objects) do
		...
		self.mdraw.x = (md.x  / #MINIONS.allies.objects) - 100
		self.mdraw.z = (md.z  / #MINIONS.allies.objects) - 100

		self.edraw.x = (em.x  / #MINIONS.enemies.objects)
		self.edraw.z = (em.z  / #MINIONS.enemies.objects)
	end

	function position:drawManager()
		DrawCircle(self.mdraw.x, myHero.y, self.mdraw.z, 400, RGB(255,0,0))
		DrawCircle(self.mdraw.x, myHero.y, self.mdraw.z, 100, RGB(255,0,59))
	end

	function position:collectTowers()
		for i = 1, objManager.maxObjects do
	        local tow = objManager:getObject(i)
	        if tow and tow.type == "obj_AI_Turret" then
	        	table.insert(self.towers, tow)
...
class 'ai'
	...
	function ai:mountBehaviours()
		...
		local se4 = Selector:new()
		local se5 = Selector:new()
		local se6 = Selector:new()

		--Squences
		local sq1 = Sequence:new()
		local sq2 = Sequence:new()
		local sq3 = Sequence:new()
		local sq4 = Sequence:new()
		local sq5 = Sequence:new()
		local sq6 = Sequence:new()
		local sq7 = Sequence:new()
		local sq8 = Sequence:new()
		...
		local EnemiesAround = Action:new{action = "EnemiesAround"}
		local EnemyMinionsNearby = Action:new{action = "EnemyMinionsNearby"}
		local PushLane = Action:new{action = "PushLane"}
		--Connections
		...
		sq2:addChild(BuyItems)

		sq3:addChild(EnemyMinionsNearby)
		sq3:addChild(PushLane)
		...
		se6:addChild(sq3)
		se6:addChild(WalkToTask)

		sq4:addChild(EnemiesAround)
...
class 'Behaviour'
	...
	function Behaviour:EnemiesAround()
		if EnemiesAround(myHero,2000) > 0 then
			return true
		else
			return false
		end
	end
	function Behaviour:PushLane()
		comboIN = false
		chasingNode = false
		AI:resetState()
		AI.state.LaneClear = true
		...
		for c,tow in pairs(POS.towers) do
	        if tow and tow.type == "obj_AI_Turret" and tow.team ~= myHero.team then
				if tow.health > 0 and not string.find(tow.name, "TurretShrine") and GetDistance(tow) < ORBWALKER:getTrueRange() then
					nearTower = tow
				end
			end
		end
		if ORBWALKER:timeToShoot() and ValidTarget(killableMinion) and ORBWALKER.CanAttack then
			attackEnemy(killableMinion)
		elseif ORBWALKER:timeToShoot() and ORBWALKER.CanAttack and nearTower then
			attackEnemy(nearTower)
		elseif lcm and ORBWALKER:timeToShoot() and ValidTarget(lcm) and ORBWALKER.CanAttack and shotWontMiss then
			myHero:Attack(lcm)
		elseif ORBWALKER:heroCanMove() then
			--ESP:getMinimumDangerRelativetoTaskFarming()
		end		
		return true
	end
	function Behaviour:EnemyMinionsNearby()
		return #MINIONS.enemies.objects > 0
	end
...
function getKillableCreep(iteration)
	...
	if minion ~= nil then
		local distanceToMinion = GetDistance(minion)
		local predictedDamage = 0
		if distanceToMinion < ORBWALKER:getTrueRange() then
			local myDamage = myHero:CalcDamage(minion, myHero.totalDamage)
			if minion.health + 1.2 - predictedDamage < myDamage then
				return minion
			end
...
function attackEnemy(enemy)
	if enemy.dead or not enemy.valid or not ORBWALKER.CanAttack then return end
	myHero:Attack(enemy)
	ORBWALKER.lastAttacked = enemy
	ORBWALKER.shotFired = true
end
...
function getHighestMinion()
	if GetTarget() ~= nil then
		...
		validTarget = ValidTarget(currentTarget, ORBWALKER:getTrueRange(), player.enemyTeam)
	...
	local highestHp = {obj = nil, hp = 0}
	for _, tMinion in pairs(MINIONS.enemies.objects) do
		if GetDistance(tMinion) <= ORBWALKER:getTrueRange() and tMinion.health > highestHp.hp then
				highestHp = {obj = tMinion, hp = tMinion.health}
		end
	end
	return highestHp.obj
...
function missChanceCalc()
	for _, tMinion in pairs(MINIONS.enemies.objects) do
		if GetDistance(tMinion) <= ORBWALKER:getTrueRange() and tMinion.health < tMinion.maxHealth/2 then
			return false
		end
	end
	return true
...
function GetTowers(team)
	local towers = {}
	for i, tower in ipairs(POS.towers) do
...
function ArrangeTSPriorities()
    ...
    ts:SetDamages((ad[myHero.charName] or mixed[myHero.charName]) and 100 or 0, (ap[myHero.charName] or mixed[myHero.charName]) and 100 or 0, 0)
    do
      local r = 0
      for i=0, 3 do
      ...
      ts.range = max(r, myHero.range+myHero.boundingRadius)
      print("System is loaded with settings of " .. myHero.charName .. ", calculated range:" .. ts.range)
    end
    local priorityOrder = {
      [1] = {1,1,1,1,1},
      [2] = {1,1,2,2,2},
...
function CheckTower()
	...
	-- Ally tower
	if GetDistance2D(DistanceTower.AllyTower, player) <= 1000 then
		underT.AllyTower = true
	else
		underT.AllyTower = false
	end
	-- Enemy tower
	if GetDistance2D(DistanceTower.EnemyTower, player) <= 1000 then
